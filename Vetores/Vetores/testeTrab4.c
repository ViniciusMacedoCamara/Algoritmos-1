...
//  Vetores
//
//  Created by Vinicius Macedo on 21/06/2018.
...
int teste(){
    ...
    int i = 0, j = 0, k = 0, v[TAM], v2[TAM], uniao[20], intersecao[20], count, aux, escolha;
    ...
    do {
        //perguntar se quer uniao ou intersecao
        printf("Deseja saber a uniao<1> ou intersecao<2>?");
        scanf("%d", &escolha);
        
        if (escolha == 1) {
            //--------- Uniao -----------
            //para saber quantos itens foram add
            count = 0;
            ...
            for(i=0; i<TAM; i++){
                ...
                for(j=0; j<TAM; j++){
                    ...
                    if(v[i] == v2[j]){
                        ...
                        if(count != 0){
                            for(k=0; k<count; k++){
                                if(v[i] != intersecao[k]){//confirmar que nao esta na intersecao
